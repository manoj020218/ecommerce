...
// ── Field refs map (for focus-on-error) ─────────────────────────────────────
...
function autoGenerateSeo(form, firstImageUrl) {
  ...
  return {
    ...
    googleShoppingTitle, googleShoppingDescription,
    ogImageUrl: firstImageUrl || ""
  };
}
...
// ── Inline quick-create (HSN / Category) ─────────────────────────────────────
...
function InlineCreate({ type, parentId, onCreated, onCancel }) {
  ...
  return (
    <div style={{ marginTop: 8, padding: "10px 12px", background: "rgba(232,35,26,0.04)", border: "1.5px solid var(--brand)", borderRadius: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: "var(--brand)", marginBottom: 8 }}>+ Quick Create {type === "hsn" ? "HSN Code" : type === "category" ? "Category" : "Subcategory"}</div>
      {type === "hsn" ? (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input placeholder="HSN Code" value={form.hsnCode} onChange={e => setForm(f => ({ ...f, hsnCode: e.target.value }))}
            style={{ flex: "0 0 100px", padding: "5px 8px", fontSize: 12, border: "1px solid var(--border)", borderRadius: 5 }} />
          <input placeholder="GST %" type="number" value={form.gstRate} onChange={e => setForm(f => ({ ...f, gstRate: e.target.value }))}
            style={{ flex: "0 0 70px", padding: "5px 8px", fontSize: 12, border: "1px solid var(--border)", borderRadius: 5 }} />
          <input placeholder="Description" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
            style={{ flex: "1 1 160px", padding: "5px 8px", fontSize: 12, border: "1px solid var(--border)", borderRadius: 5 }} />
          <input type="date" value={form.effectiveFrom} onChange={e => setForm(f => ({ ...f, effectiveFrom: e.target.value }))}
            style={{ flex: "0 0 130px", padding: "5px 8px", fontSize: 12, border: "1px solid var(--border)", borderRadius: 5 }} />
        </div>
      ) : (
        <input placeholder="Name" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
          style={{ width: "100%", padding: "5px 8px", fontSize: 12, border: "1px solid var(--border)", borderRadius: 5, boxSizing: "border-box" }} />
      )}
      {err && <p style={{ color: "var(--danger)", fontSize: 11, margin: "5px 0 0" }}>{err}</p>}
      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
        <button type="button" className="btn btn-primary btn-small" onClick={submit} disabled={saving} style={{ fontSize: 12 }}>
    ...
    <div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
          {selected.map(p => (
            <span key={p.id} style={{
              ...
              borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 500, color: "#1d4ed8"
            }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        {open && suggestions.length > 0 && (
          <div ref={dropRef} style={{
            ...
            background: "var(--surface)", border: "1px solid var(--border)",
            borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.1)", maxHeight: 240, overflowY: "auto"
          }}>
            {suggestions.map(p => {
              const firstImg = resolveImageUrl(p.images?.[0]);
              return (
                <div key={p.id} onMouseDown={() => add(p)} style={{
                ...
                >
                  {firstImg
                    ? <img src={firstImg} alt="" style={{ width: 32, height: 32, objectFit: "cover", borderRadius: 4, flexShrink: 0, border: "1px solid var(--border)" }} />
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "var(--muted)" }}>
                      {[p.sku, p.brand, p.hsnCode && `HSN ${p.hsnCode}`].filter(Boolean).join(" · ")}
                    </div>
                  </div>
                  <div style={{ marginLeft: "auto", fontSize: 12, fontWeight: 600, color: "var(--text)", whiteSpace: "nowrap" }}>
        ...
        {open && query.trim() && suggestions.length === 0 && (
          <div style={{
            ...
            padding: "12px", fontSize: 13, color: "var(--muted)"
          }}>
      ...
      {selected.length > 0 && (
        <p style={{ margin: "5px 0 0", fontSize: 11, color: "var(--muted)" }}>
          {selected.length} related product{selected.length !== 1 ? "s" : ""} selected
        </p>
      )}
    </div>
  );
...
function TagsInput({ tags, onChange }) {
  ...
  return (
    <div style={{
      ...
      border: "1px solid var(--border)", borderRadius: 7, minHeight: 38,
      ...
      {tags.map(tag => (
        <span key={tag} style={{
          ...
          borderRadius: 20, padding: "2px 9px", fontSize: 12, fontWeight: 500, color: "#334155"
        }}>
          {tag}
          <button type="button" onClick={() => onChange(tags.filter(t => t !== tag))}
            style={{ background: "none", border: "none", cursor: "pointer", color: "#64748b", fontSize: 13, lineHeight: 1, padding: 0 }}>×</button>
        </span>
      ))}
      <input
        value={input}
...
// ── Video panel (reused from products-page) ───────────────────────────────────
...
function VideoPanel({ videos, onChange }) {
  ...
  const add = () => {
    ...
    onChange([...videos, { url: newUrl.trim(), label: newLabel.trim() }]);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {videos.map((v, i) => (
        <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <input value={v.url} onChange={e => onChange(videos.map((x, j) => j === i ? { ...x, url: e.target.value } : x))}
            style={{ flex: 1, padding: "5px 8px", fontSize: 12, border: "1px solid var(--border)", borderRadius: 5 }} placeholder="URL" />
          <input value={v.label || ""} onChange={e => onChange(videos.map((x, j) => j === i ? { ...x, label: e.target.value } : x))}
            style={{ width: 130, padding: "5px 8px", fontSize: 12, border: "1px solid var(--border)", borderRadius: 5 }} placeholder="Label" />
          <button type="button" onClick={() => remove(i)}
            style={{ background: "#fee2e2", border: "none", borderRadius: 5, padding: "4px 10px", cursor: "pointer", color: "#ef4444", fontWeight: 700 }}>×</button>
        </div>
      ))}
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <input value={newLabel} onChange={e => setNewLabel(e.target.value)}
          style={{ width: 160, fontSize: 13, padding: "6px 10px", border: "1px solid var(--border)", borderRadius: 6 }} placeholder="Label (optional)" />
...
function DownloadsPanel({ downloads, onChange }) {
  ...
  const add = () => {
    ...
    onChange([...downloads, { title: title.trim(), url: url.trim() }]);
    setTitle(""); setUrl("");
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {downloads.map((d, i) => (
        <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button type="button" onClick={() => remove(i)}
            style={{ background: "#fee2e2", border: "none", borderRadius: 5, padding: "2px 8px", cursor: "pointer", color: "#ef4444", fontWeight: 700 }}>×</button>
        </div>
      ))}
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
        <input value={title} onChange={e => setTitle(e.target.value)}
          style={{ flex: "0 0 160px", fontSize: 13, padding: "5px 8px", border: "1px solid var(--border)", borderRadius: 5 }} placeholder="Title (e.g. Datasheet)" />
        <input value={url} onChange={e => setUrl(e.target.value)}
          onKeyDown={e => { if (e.key === "Enter") { e.preventDefault(); add(); } }}
          style={{ flex: 1, fontSize: 13, padding: "5px 8px", border: "1px solid var(--border)", borderRadius: 5 }} placeholder="https://..." />
        <button type="button" onClick={add} className="btn btn-secondary btn-small">+ Add Link</button>
      </div>
      <p style={{ fontSize: 11, color: "var(--muted)", margin: 0 }}>File upload available after saving. Add URL links now.</p>
    </div>
  );
}
...
const EMPTY_FORM = {
  ...
  deadWeightKg: 0, lengthCm: "", widthCm: "", heightCm: "", shippingClass: "normal",
  videos: [], downloads: [],
  metaTitle: "", metaDescription: "", metaKeywords: "",
...
export function AddProductPage() {
  ...
  const focusFirstError = (msg, ferrs) => {
    ...
    if (errField && fieldRefs.current[errField]) {
      fieldRefs.current[errField].focus();
      fieldRefs.current[errField].scrollIntoView({ behavior: "smooth", block: "center" });
      return;
    }
    // Try message-based mapping
    for (const [keyword, name] of FIELD_ERROR_MAP) {
      if (lower.includes(keyword) && fieldRefs.current[name]) {
  ...
  const onAutoSeo = () => {
    const firstImgUrl = pendingImages[0]?.previewUrl || "";
    const seo = autoGenerateSeo(form, firstImgUrl);
    setForm(f => ({ ...f, ...seo }));
  };
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      quoteRequiredAboveQty: form.quoteRequiredAboveQty === "" || form.quoteRequiredAboveQty === null
      ...
      relations: form.relatedProductIds.length
        ? { related: form.relatedProductIds }
        : undefined
    };
    ...
    try {
      ...
      if (newId && pendingImages.length > 0) {
        for (const { file } of pendingImages) {
          try { await uploadProductImage(newId, file); } catch { /* skip */ }
        }
  ...
  if (!canCreate) {
    return (
      <div style={{ padding: 32, color: "var(--danger)" }}>You do not have permission to create products.</div>
    );
  }
  ...
  return (
    <div style={{ padding: "24px 28px", background: "var(--bg)", minHeight: "100vh" }}>
      ...
      {/* Error banner */}
      {error && (
        <div id="page-error-banner" style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16, alignItems: "start" }}>
          ...
          <div>
            ...
            <Section title="Core Details">
              <Field label="Title" required error={fieldErrors.title}>
                <input ref={registerRef("title")} name="title" value={form.title} onChange={onFC} onBlur={titleBlurred}
                  required style={inputStyle(fieldErrors.title)} placeholder="Product title…" />
              </Field>
              <FieldRow>
                <Field label="Slug" hint="(auto-filled from title)">
                  <input ref={registerRef("slug")} name="slug" value={form.slug} onChange={onFC}
                    style={inputStyle()} placeholder="product-slug" />
                </Field>
              ...
              <FieldRow>
                <Field label="Brand">
                  <input ref={registerRef("brand")} name="brand" value={form.brand} onChange={onFC}
                    style={inputStyle(fieldErrors.brand)} placeholder="Jenix, Siemens…" />
                </Field>
                <Field label="Model Number">
                  <input ref={registerRef("modelNumber")} name="modelNumber" value={form.modelNumber} onChange={onFC}
                    style={inputStyle()} placeholder="Model / Part no." />
                </Field>
              </FieldRow>
              <FieldRow>
                {/* Category */}
                <div>
                  <Field label="Category" error={fieldErrors.categoryId}>
                    <div style={{ display: "flex", gap: 6 }}>
                      <select ref={registerRef("categoryId")} name="categoryId" value={form.categoryId} onChange={onFC}
                        style={{ ...selectStyle(fieldErrors.categoryId), flex: 1 }}>
                        <option value="">None</option>
                        {categories.filter(c => !c.parentCategoryId).map(c => (
                          <option key={c.id} value={c.id}>{c.name}</option>
                        ))}
                      </select>
                      <button type="button" title="Quick create category"
                        onClick={() => setInlineCreate(inlineCreate === "category" ? null : "category")}
                        style={{ padding: "0 10px", fontSize: 16, border: "1px solid var(--border)", borderRadius: 7, background: "var(--surface)", cursor: "pointer", color: "var(--brand)", fontWeight: 700 }}>+</button>
                    </div>
                  </Field>
                  {inlineCreate === "category" && (
                    <InlineCreate type="category" onCreated={async (id) => {
                      const fresh = await fetchCategories({ includeInactive: true, q: "" });
                ...
                <div>
                  <Field label="Subcategory">
                    <div style={{ display: "flex", gap: 6 }}>
                      <select name="subcategoryId" value={form.subcategoryId} onChange={onFC}
                        style={{ ...selectStyle(), flex: 1 }} disabled={!form.categoryId}>
                        <option value="">None</option>
                        {subcategoryOptions.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                      </select>
                      {form.categoryId && (
                        <button type="button" title="Quick create subcategory"
                          onClick={() => setInlineCreate(inlineCreate === "subcategory" ? null : "subcategory")}
                          style={{ padding: "0 10px", fontSize: 16, border: "1px solid var(--border)", borderRadius: 7, background: "var(--surface)", cursor: "pointer", color: "var(--brand)", fontWeight: 700 }}>+</button>
                      )}
                    </div>
                  </Field>
                  {inlineCreate === "subcategory" && (
                    <InlineCreate type="subcategory" parentId={form.categoryId} onCreated={async (id) => {
                      const fresh = await fetchCategories({ includeInactive: true, q: "" });
                      setCategories(Array.isArray(fresh) ? fresh : []);
                      if (id) set("subcategoryId", id);
                      setInlineCreate(null);
                    }} onCancel={() => setInlineCreate(null)} />
                  )}
                </div>
              ...
              <div>
                <Field label="HSN Code" required error={fieldErrors.hsnCode}>
                  <div style={{ display: "flex", gap: 6 }}>
                    <select ref={registerRef("hsnCode")} name="hsnCode" value={form.hsnCode} onChange={onFC}
                      required style={{ ...selectStyle(fieldErrors.hsnCode), flex: 1 }}>
                      <option value="">Select HSN…</option>
                      {hsnRecords.map(h => (
                        <option key={h.hsnCode} value={h.hsnCode}>
                          {h.hsnCode} – {h.gstRate}% – {(h.description || "").slice(0, 50)}
                        </option>
                      ))}
                    </select>
                    <button type="button" title="Quick create HSN"
                      onClick={() => setInlineCreate(inlineCreate === "hsn" ? null : "hsn")}
                      style={{ padding: "0 10px", fontSize: 16, border: "1px solid var(--border)", borderRadius: 7, background: "var(--surface)", cursor: "pointer", color: "var(--brand)", fontWeight: 700 }}>+</button>
                  </div>
                </Field>
                {inlineCreate === "hsn" && (
                  <InlineCreate type="hsn" onCreated={async (code) => {
                    const fresh = await fetchHsnTaxRecords({ includeInactive: false, q: "" });
                    setHsnRecords(Array.isArray(fresh) ? fresh : []);
                    set("hsnCode", code);
                    setInlineCreate(null);
                  }} onCancel={() => setInlineCreate(null)} />
                )}
            ...
            <Section title="Pricing & Inventory">
              <FieldRow cols={3}>
                <Field label="Base Price (₹)" required error={fieldErrors.basePrice}>
                  <input ref={registerRef("basePrice")} type="number" min="0" step="0.01" name="basePrice"
                    value={form.basePrice} onChange={onFC} required style={inputStyle(fieldErrors.basePrice)} />
                </Field>
                <Field label="Sale Price (₹)">
                  <input ref={registerRef("salePrice")} type="number" min="0" step="0.01" name="salePrice"
                    value={form.salePrice} onChange={onFC} style={inputStyle()} />
                </Field>
                <Field label="MOQ">
              ...
              <div style={{ display: "flex", gap: 20, flexWrap: "wrap", marginBottom: 12 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13 }}>
                  <input type="checkbox" name="bulkPricingEnabled" checked={form.bulkPricingEnabled} onChange={onFC} />
              ...
              <Field label="Bulk Price Slabs" hint="(minQty:price, e.g. 10:2450, 25:2300)">
                <input name="bulkPriceSlabsText" value={form.bulkPriceSlabsText} onChange={onFC}
                  style={inputStyle()} placeholder="10:2450, 25:2300" />
              </Field>
              <FieldRow>
                <Field label="Price Group Prices" hint="(group:price)">
                  <input name="priceGroupPricesText" value={form.priceGroupPricesText} onChange={onFC}
                    style={inputStyle()} placeholder="dealer:4100, stockist:3950" />
                </Field>
                <Field label="Customer Specific Prices" hint="(customerId:price)">
                  <input name="customerSpecificPricesText" value={form.customerSpecificPricesText} onChange={onFC}
                    style={inputStyle()} placeholder="user_abc:3890" />
                </Field>
              </FieldRow>
            </Section>
            ...
            <Section title="Shipping & Dimensions">
              <FieldRow cols={4}>
                <Field label="Weight (kg)">
                  <input ref={registerRef("deadWeightKg")} type="number" min="0" step="0.01" name="deadWeightKg"
                    value={form.deadWeightKg} onChange={onFC} style={inputStyle(fieldErrors.deadWeightKg)} />
                </Field>
              ...
              <div style={{ marginTop: 12 }}>
                <FieldRow>
                  <Field label="Technical Keywords" hint="(comma separated)">
                    <input name="technicalKeywordsText" value={form.technicalKeywordsText} onChange={onFC}
                      style={inputStyle()} placeholder="relay, 24vdc, 10a, din rail" />
                  </Field>
                  <Field label="Customer Keywords" hint="(comma separated)">
                    <input name="customerKeywordsText" value={form.customerKeywordsText} onChange={onFC}
                      style={inputStyle()} placeholder="industrial relay, panel relay" />
                  </Field>
                </FieldRow>
                <FieldRow>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text)", marginBottom: 8, textTransform: "uppercase" }}>
                  ...
                  {pendingImages.length > 0 && (
                    <span style={{ marginLeft: 8, fontSize: 11, color: "#16a34a", fontWeight: 400, textTransform: "none" }}>
                      {pendingImages.length} queued — uploads after save
                    </span>
                  )}
                </div>
                  ...
                  {pendingImages.map((p, i) => (
                    <div key={i} style={{ position: "relative" }}>
                      ...
                      <button type="button" onClick={() => removeImage(i)}
                        style={{ position: "absolute", top: -6, right: -6, width: 20, height: 20, borderRadius: "50%", background: "#ef4444", color: "#fff", border: "none", fontSize: 11, cursor: "pointer", lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
                    </div>
                  ))}
            ...
            <Section title={
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                SEO &amp; Meta
                <button type="button" onClick={onAutoSeo}
                  style={{ fontSize: 12, fontWeight: 600, padding: "3px 12px", background: "var(--brand)", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer" }}>
                  ✦ Auto-Generate SEO
                </button>
              </div>
            }>
              <Field label={`Meta Title (${(form.metaTitle || "").length}/60)`} error={fieldErrors.metaTitle}>
                <input ref={registerRef("metaTitle")} name="metaTitle" value={form.metaTitle} onChange={onFC}
                  maxLength={120} style={inputStyle(fieldErrors.metaTitle)} placeholder="Product Title | Brand" />
              </Field>
              <Field label={`Meta Description (${(form.metaDescription || "").length}/155)`} error={fieldErrors.metaDescription}>
                <textarea ref={registerRef("metaDescription")} name="metaDescription" value={form.metaDescription}
          ...
          <div style={{ position: "sticky", top: 20, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px" }}>
              <h3 style={{ margin: "0 0 4px", fontSize: 13, fontWeight: 700, color: "var(--text)", textTransform: "uppercase", letterSpacing: "0.4px" }}>
                Hand Picked Related Products
              </h3>
              <p style={{ margin: "0 0 10px", fontSize: 11, color: "var(--muted)" }}>
                Type a product name or SKU — select to link
              </p>
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px" }}>
              ...
              <p style={{ margin: "0 0 10px", fontSize: 11, color: "var(--muted)" }}>
                Comma or Enter to add. Used for quick search recall.
              </p>
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px" }}>
              ...
              <p style={{ margin: "0 0 10px", fontSize: 11, color: "var(--muted)" }}>
                Badge shown on product card, e.g. "New", "Best Seller", "Sale"
              </p>
              <input
                name="productLabel"
                value={form.productLabel}
                onChange={onFC}
                placeholder="New Arrival, Best Seller, Hot…"
                style={inputStyle()}
              />
            </div>
            ...
            {(form.metaTitle || form.title) && (
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px" }}>
                ...
                <div style={{ padding: "10px 12px", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 6 }}>
                  <div style={{ fontSize: 14, color: "#1a0dab", fontWeight: 500, marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {form.metaTitle || form.title}
                  </div>
                  ...
                  <div style={{ fontSize: 12, color: "#545454", lineHeight: 1.5 }}>
                    {form.metaDescription || stripHtml(form.shortDescription).slice(0, 120) || "No description yet."}
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Quick tips */}
            <div style={{ background: "rgba(37,99,235,0.04)", border: "1px solid rgba(37,99,235,0.15)", borderRadius: 10, padding: "14px 16px" }}>
              <h3 style={{ margin: "0 0 8px", fontSize: 11, fontWeight: 700, color: "#1d4ed8", textTransform: "uppercase" }}>Tips</h3>
              <ul style={{ margin: 0, padding: "0 0 0 16px", fontSize: 11, color: "#1e40af", lineHeight: 1.8 }}>
                <li>Title auto-fills the slug on blur</li>
                <li>Use ✦ Auto-Generate SEO after filling title &amp; description</li>
                <li>Images queue here, upload after save</li>
                <li>Related products show in "You may also like"</li>
              </ul>
            </div>
