...
- [ ] Admin-side wishlist visibility — buyer "Save product" (❤) is fully working end-to-end and shows in the buyer's own `/account` "Saved Products" card, but admin has **zero visibility** into it: confirmed `sanitizeAdminCustomer` in `backend/src/modules/customers/customers.service.js` returns `savedAddresses` but never `savedProductIds`, and nothing in the admin panel references saved products anywhere. Planned fix (not started): surface it on the admin Customer Detail page (list/count of saved products), and consider a per-product "saved by N customers" count on the Products list. User explicitly asked to defer this to a future session.
