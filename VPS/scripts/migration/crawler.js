...
/**
 * commmerce.com builder puts both prices in one container:
 * "₹ 1,390    ₹ 4,199  (Excluding 18 % tax)"
 * Returns { salePrice, mrp } parsed from ₹ occurrences.
 */
...
/** True for actual product images, false for UI icons / SVGs. */
...
/** Collect all src/data-src image URLs from matching elements. */
...
/** Parse specifications from a table or definition list. */
function parseSpecs($, selectors) {
  ...
  for (const sel of selectors) {
    try {
      ...
      // Table rows: <tr><td>key</td><td>value</td></tr>
      ...
      // Definition list: <dt>key</dt><dd>value</dd>
  ...
  for (const sitePath of SITEMAP_PATHS) {
    ...
    try {
      ...
      if (nestedUrls.length > 0) {
        console.log(`  [sitemap] Index found with ${nestedUrls.length} nested sitemaps`);
        for (const nestedUrl of nestedUrls) {
          if (nestedUrl.toLowerCase().includes("product")) {
            try {
              ...
              console.log(`  [sitemap] Nested: ${nestedUrl} → ${productUrls.size} products so far`);
            } catch {
              // continue
            }
            await sleep(500);
      ...
      if (productUrls.size > 0) {
        console.log(`  [sitemap] Found ${productUrls.size} product URLs in ${sitePath}`);
        break;
      }
    } catch (err) {
...
async function discoverFromCategories(page, existingUrls) {
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      // Also check all links for category pages
      $("a[href]").each((_, el) => {
        const href = absoluteUrl($(el).attr("href"), url);
        if (!href || !sameDomain(href) || isBlocked(href)) return;
        if (
          isProductUrl(href) &&
          !productUrls.has(href)
        ) {
          productUrls.add(href);
          found++;
        }
      ...
      console.log(
        `  [discovery] ${url} → +${found} products (total: ${productUrls.size})`
      );
      ...
      for (const sel of SELECTORS.nextPage) {
        try {
          ...
          if (nextHref && sameDomain(nextHref) && !visitedPages.has(nextHref)) {
            queue.push(nextHref);
          }
        } catch {
          continue;
      ...
      // Collect category links (links that look like category pages)
      $("a[href]").each((_, el) => {
        const href = absoluteUrl($(el).attr("href"), url);
        if (!href || !sameDomain(href) || isBlocked(href)) return;
        const { pathname } = new URL(href);
        const isCategory =
          /\/(category|categories|collection|collections|shop|store|products)\//i.test(
            pathname
          ) ||
          (/\/(cat)\//i.test(pathname));
        if (isCategory && !visitedPages.has(href)) {
          queue.push(href);
        }
      });
    } catch (err) {
...
export async function scrapePage(page, url) {
  ...
  // Try commmerce.com-specific price extraction first
  ...
  // Fallback to main image if no gallery images found
  if (rawImageUrls.length === 0) {
    const mainSrc = collectImages($, SELECTORS.mainImage);
    rawImageUrls.push(...mainSrc);
  }
  // Also try og:image (filter for actual product images)
  ...
  // Extract key features from bullet lists near description
  const keyFeatures = [];
  $(".key-features li, .product-features li, .features li, ul.features li").each(
    (_, el) => {
...
/** Discover all product URLs and save to urls.json */
export async function discoverUrls(outputDir) {
  ...
  try {
    ...
    console.log(`[discover] Done. ${unique.length} product URLs saved → ${urlsFile}`);
    return unique;
  } finally {
    await browser.close();
  }
...
/** Crawl product pages and save to products_raw.json / images/ */
export async function crawlProducts(outputDir, imageDir, limit = Infinity) {
  ...
  // Load already-crawled products to support resume
