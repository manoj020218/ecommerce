...
// Turns one Zod issue into a { formField, label, message } triple the save
// handler can both display and focus by -- this is what actually surfaces
// *which* field and *why* a save failed instead of the bare "Validation
// failed." the backend's top-level error message carries (the useful detail
// only exists in payload.details.issues, which nothing was reading before).
...
export function EditProductPage() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setError(msg);
      const lower = msg.toLowerCase();
      for (const [keyword, name] of FIELD_ERROR_MAP) { if (lower.includes(keyword) && !fe[name]) { fe[name] = msg; break; } }
      setFieldErrors(fe);
